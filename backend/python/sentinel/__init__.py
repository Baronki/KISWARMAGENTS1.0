...
# ── v4.7-4.8: Swarm Networking Modules (v6.3.5) ─────────────────────────────────
from .swarm_peer import SwarmPeer, PeerInfo, PeerState, PeerConnection, MsgType
from .feedback_channel import FeedbackChannel, KnownFix
from .gossip_protocol import GossipProtocol, GossipItem, GossipType

# ── v4.4: Swarm Auditor & DAG (v6.3.5) ─────────────────────────────────────────
from .swarm_auditor import (
    AuditLedger, PipelineDAG, DAGNode, DAGEdge, DAGIssues,
    PIPELINES, run_audit_cycle, run_pipeline_step,
    run_pfd_calculation, run_sil_band_check, repair_dag,
    load_pipeline_dag, save_pipeline_dag, populate_dummy_data,
    validate_dag_consistency,
)
from .swarm_dag import (
    SwarmAuditorNode, PermanentAuditor, SwarmCoordinator,
)

# ── v2.2: Crypto Ledger (Module 4) ─────────────────────────────────────────────
from .crypto_ledger import CryptoLedger, CryptographicKnowledgeLedger, LedgerEntry, TamperReport

# ── v4.1: Byzantine Aggregator (Module 22) ─────────────────────────────────────
from .byzantine_aggregator import ByzantineFederatedAggregator, SiteUpdate, AggregationResult

# ── v5.0: HexStrike Guard (Module 31) ──────────────────────────────────────────
from .hexstrike_guard import HexStrikeGuard, ToolRegistry, AgentTask, GuardReport

# ── v2.2: Prompt Firewall (Module 6) ───────────────────────────────────────────
from .prompt_firewall import AdversarialPromptFirewall, FirewallReport, ThreatType
...
__all__ = [
    ...
    # v4.7-4.8: Swarm Networking (v6.3.5)
    "SwarmPeer", "PeerInfo", "PeerState", "PeerConnection", "MsgType",
    "FeedbackChannel", "KnownFix",
    "GossipProtocol", "GossipItem", "GossipType",
    ...
    # Security Modules (v6.3.5)
    "CryptoLedger",
    "ByzantineFederatedAggregator", "SiteUpdate", "AggregationResult",
    "HexStrikeGuard", "ToolRegistry", "AgentTask", "GuardReport",
]
